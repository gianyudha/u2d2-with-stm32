...
| Resistor 100 Ω | 1 | Series protection on PA9 |
...
|:--:|:--:|:--:|:--:|:--:|:--:|:--|
| 1 | PA11 | USB_DM | AF | — | USB | USB CDC (D−) |
| 2 | PA12 | USB_DP | AF | — | USB | USB CDC (D+) |
| 3 | PA9 | USART1_TX | AF / Open-Drain | NO PULL | DYNAMIXEL | Single-wire half-duplex data; ext. 4.7 kΩ pull-up to **3.3 V** + 100 Ω series |
| 4 | PC13 | LED | OUT | LOW | LED | Onboard heartbeat (toggles on TX complete; optional) |
| 5 | — | HSE | — | 25 MHz | CLOCK | Crystal → 48 MHz exact for USB |

### Servo / Power Connection

| FROM | VIA | TO | NOTE |
|:--|:--:|:--|:--|
| PA9 | 100 Ω series, then 4.7 kΩ pull-up to 3.3 V | DATA (middle pin MX-28T) | Protection mandatory |
...
```
                          3.3 V
                            │
                         [4.7 kΩ]   ← pull-up (to 3.3 V, NOT 5 V)
                            │
 PA9 ──[100 Ω series]───────┴──────────── DATA (middle pin of MX-28T)

 12 V (+) ───────────────────────────────  VDD (servo power pin)

 GND ──┬── GND BlackPill
       ├── GND servo
       └── GND 12 V supply        ← COMMON GROUND (mandatory, thick/short wire)
```
...
2. **Add a 100 Ω series resistor on PA9.**
   Limits transient/inrush current into the pin when the servo draws current. Cheap, effective, and the single most important protection against back-EMF / ground-bounce spikes on the data line.

3. **Common ground is mandatory.** Servo GND, BlackPill GND, and 12 V supply GND must meet at one solid point with thick, short wire. A floating or thin ground forces motor return current through the PA9 data line.

4. **Servo power is separate.** Never power the servo from the BlackPill's USB 5 V. Only the ground is shared.
...
port.openPort()
port.setBaudRate(1000000)             # 1 Mbps
# read/write as with a real U2D2 ...
```

### ROS / ROS 2
The bridge is indistinguishable from a real U2D2 to ROS drivers. Just point the driver at the port and baud:
```yaml
port_name: /dev/ttyACM0
baud_rate: 1000000
```
- **ROS 2:** `dynamixel_hardware` (ros2_control) or the ROBOTIS DYNAMIXEL SDK examples.
- **ROS 1:** `dynamixel_workbench`.

> For high-frequency multi-servo `GroupSyncRead`, make sure the combined response still fits in the RX buffer (raise `RX_BUF_SIZE` if needed).
...
MIT (or your choice). Firmware skeleton generated by STM32CubeIDE is under STMicroelectronics' terms; see headers.
